...
The acceptance suites — the ones that load a model or replay real meeting audio — are off by default: they need `TEST_RUNNER_ECHO_ACCEPTANCE=1` and audio fixtures that are not in the repository, and they skip themselves with a note when either is missing. The fixtures are real hardware recordings, which is why they aren't checked in — but a debug build can record its own: [Fixtures/README.md](Fixtures/README.md) has the scenarios, what each take asks you to do, and which ones need hardware you probably don't have. Tests are hosted in `Echo.app` and share its data folder, so don't run them while your own Echo is recording.
