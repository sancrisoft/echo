...
struct EnergyEnvelopeTests {
    ...
    private let rate = TranscriptionPass.sampleRate
    ...
    /// Direct sqrt(mean of squares) over a whole buffer — the reference the
    /// envelope has to agree with. Local to this target on purpose: rms over a
    /// raw capture buffer belongs to the audio package, which Transcription
    /// must never depend on.
    private func directRMS(_ samples: [Float]) -> Float {
...
@Suite("TranscriptionPass span levels")
struct ParakeetSpanLevelsTests {
    ...
    private func segment(_ channel: AudioChannel, start: Double, end: Double) -> TranscriptSegment {
        TranscriptSegment(
            channel: channel,
            speaker: EchoCore.Speaker(defaultFor: channel),
            text: "synthetic",
            start: start,
            end: end
        )
    ...
    func ownAndOtherAreOrientedByChannel() {
        ...
        let levels = TranscriptionPass.spanLevels(
    ...
    func halfMeasuredSpansAreOmitted() {
        ...
        #expect(TranscriptionPass.spanLevels(of: [micSegment], envelopes: envelopes).isEmpty)
    }
    ...
    func spansPastTheShorterChannelAreOmitted() {
        ...
        #expect(TranscriptionPass.spanLevels(of: [late], envelopes: envelopes).isEmpty)
    }
}
