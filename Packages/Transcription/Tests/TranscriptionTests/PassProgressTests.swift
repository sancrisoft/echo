...
@Suite("PassProgress")
struct PassProgressTests {

    @Test("no retained audio reads complete immediately")
    func emptyInputIsComplete() {
        #expect(PassProgress(channelTotalSamples: []).fraction == 1.0)
        #expect(PassProgress(channelTotalSamples: [0, 0]).fraction == 1.0)
    }

    @Test("the fraction starts at zero and advances with decoded windows")
    func advancesWithWindows() {
        var progress = PassProgress(channelTotalSamples: [1_000])
    ...
    func bothChannelsWeighting() {
        ...
        var progress = PassProgress(channelTotalSamples: [1_000, 3_000])
    ...
    func silentChannelCompletes() {
        ...
        var progress = PassProgress(channelTotalSamples: [500, 500])
    ...
    func clampsOverrun() {
        var progress = PassProgress(channelTotalSamples: [100])
