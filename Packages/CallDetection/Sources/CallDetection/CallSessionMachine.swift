...
//  CallDetection
//
//  The whole behavior of the call-detection island — when it appears, which
//  face it wears, when a recording is offered, and when a forgotten one stops
//  — as a pure, exhaustively table-tested machine.
//
//  Structure mirrors `Audio.InputDeviceLifecycleMachine`: Core Audio and the
//  panel stay thin and manual; every decision, suppression rule and timer edge
//  lives here.
//
...
/// The island's timing constants. Defined once so the machine, the
/// controller's timers and the tests can never disagree.
public enum CallDetectionTiming {
    ...
    public static let startDebounce: TimeInterval = 3
...
/// happening.
///
/// Four faces, as the PoC shipped them. The redesign calls for six; growing
/// this is the island's own work, not the port's.
public enum IslandFace: Equatable, Sendable {

    /// The one face that offers to record. `appName` is island copy only,
    /// never persisted as meeting metadata. `scoped` is whether the tap will
    /// narrow the session to that app — set from the same derivation that
    /// picks the scope, so the button's promise ("Record Zoom" vs "Start
    /// recording") can never disagree with what the tap requests.
...
/// window. It cannot touch capture, persistence, or the summary path — the
/// island's blast radius is unrepresentable beyond "the same start/stop the
/// menu bar runs".
public struct CallSessionMachine {
    ...
    public enum Action: Equatable, Sendable {
        ...
        /// Run the same gated start every other surface runs, narrowed to the
        /// carried scope: the app the island names, or `.everything` when it
        /// names none.
        case requestStartRecording(CaptureScope)
        /// Run the same stop every other surface runs, its sequencing
        /// inherited rather than reimplemented.
        case requestStopRecording
        case openWindowToSavedMeeting
    }

    /// The feature's setting: on by default. Off means inert — no face, no
    /// timers, no requests.
    ...
    public private(set) var currentApp: ProcessSelector?

    /// Mirrors whether a recording is running. Tracked even while the feature
    ...
    /// call prompts again.
    public private(set) var dismissedThisCall = false
    ...
    public private(set) var keptRecordingLatch = false
    ...
    /// The scope the island's start tap will request, in one expression: a
    /// scopeable attributed app records that app (the one the island names —
    /// the first catalog match); an unscopeable one (the FaceTime daemon) or
    /// an app-less call runs as an honest Everything. `promptFace` derives its
    /// `scoped` flag from this same value, so the button copy and the capture
    ...
    private mutating func handleMatchedApps(_ apps: [ProcessSelector]) -> [Action] {
        switch phase {
        case .idle:
        ...
        case .inCall:
            ...
            guard !keptRecordingLatch else {
                // Defense in depth: a call whose end the user chose to record
                // through never raises a second countdown. Confirming a call
                // clears the latch, so this branch is unreachable today — it
                // exists so the rule survives a future path into `inCall` that
                // skips the debounce.
                endCall()
                return [.setFace(nil)]
            }
    ...
    private mutating func handleRecordingChanged(_ recording: Bool) -> [Action] {
        ...
        case .inCall:
            ...
            // is not offered a recording again.
