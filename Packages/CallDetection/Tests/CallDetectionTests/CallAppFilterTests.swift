...
//  CallDetectionTests
//
//  Per-app call detection: the disabled-apps filter sits at the single matcher
//  call site (`CallAppCatalog.matchedApps`), keyed on display names — one name
//  covers all of an app's bundle prefixes, a stale name is harmlessly ignored,
//  and an empty set is byte-for-byte plain matching. What this function
//  returns IS what feeds the machine's `matchedAppsChanged`, so a filtered-out
//  app can never reach the island or the scope dropdown.
//
...
struct CallAppFilterTests {
    ...
    private func client(_ bundleID: String, pid: pid_t = 100) -> MicCaptureClient {
        MicCaptureClient(pid: pid, bundleID: bundleID)
    }

    @Test func anEmptyDisabledSetMatchesExactlyAsToday() {
        let clients = [client("us.zoom.xos"), client("com.hnc.Discord", pid: 101)]
        let apps = CallAppCatalog.matchedApps(from: clients)
