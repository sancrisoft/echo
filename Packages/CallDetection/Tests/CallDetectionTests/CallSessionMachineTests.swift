...
//  CallDetectionTests
//
//  The island's entire behavior contract, one test per row of the machine's
//  transition table — plus the two non-negotiable product lines as sweeps over
//  every short event sequence:
//
//    • nothing records without an explicit tap, and
//    • no start prompt ever appears over a running recording.
//
//  The Core Audio shim and the panel stay manual; everything decided lives
//  here.
//
...
struct CallSessionMachineTests {
    ...
    @Test func openEchoOpensTheSavedMeetingAndHidesTheIsland() {
        ...
        #expect(actions == [.cancelRetractTimer, .setFace(nil), .openWindowToSavedMeeting])
        #expect(machine.face == nil)
