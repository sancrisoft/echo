...
//  (`com.brave.Browser.helper.renderer`), so `ProcessSelector.matches` finds
//  them from the bundle ID alone. Gecko-based browsers do not work that way:
//  Zen's media process is `app.zen-browser.plugincontainer` and Firefox's is
//  `org.mozilla.plugincontainer` — neither shares a prefix with the browser
//  the user actually launched. What every architecture *does* share is a
//  path: the helper's executable lives inside the parent's bundle. So the
//  process's executable path, walked up to its outermost `.app`, is the one
//  identity that holds for all of them.
//
//  This is what makes browser detection browser-agnostic: paired with
//  CallDetection's browser catalog, a mic-capturing process is recognised as
//  "a browser" by the bundle it lives in, so a browser Echo has never heard
//  of is detected the day it is installed.
//
...
public enum AppBundleIdentity {
    ...
    /// executable, or `""` when there is none: daemons, XPC services that
    /// live inside a framework rather than an app (Safari's WebKit GPU
    /// process is one — it keeps its curated catalog entry for exactly this
    /// reason), and pids that died before the read.
    ///
    ...
    ///
    /// `nil` for a path with no `.app` component at all, so a daemon is
    /// never read as belonging to an app.
    ...
    /// The process's executable path. `proc_pidpath` needs no entitlement
    /// for processes of the same user, and Echo runs unsandboxed (system-audio
    ...
    ///
    /// Behind a `Mutex` because the callers are threads, not actors: a scoped
    /// tap's follow update runs on the main queue while CallDetection's
    /// mic-activity monitor resolves processes on its own queue, and neither
    /// may block on the other's isolation to read a cached string.
    ...
    private static func bundleID(atPath path: String) -> String {
        ...
        let resolved = Bundle(url: URL(filePath: path))?.bundleIdentifier ?? ""
        cache.withLock { $0[path] = resolved }
        return resolved
    }
}
