...
//  Audio
//
//  The live input-health signal source. The two 16 kHz mono Float streams are
//  buffered per channel and cut at natural pauses (energy-based endpointing),
//  and every finalized chunk is measured and gate-classified — but nothing
//  decodes. There is no model here, nothing to load, nothing to wait for, and
//  NO text is produced during a recording: a live transcript is not something
//  this product shows.
//
//  The chunking exists because the gate decisions do: `GateDecisionRecord` is
//  what `InputHealthTracker` consumes to raise the input-health warnings
//  ("your mic is being discarded", "the Others channel went quiet"), and
//  those verdicts are only meaningful over utterance-shaped chunks.
//
//  Recording therefore never depends on any model: `ingest` accepts audio
//  from the first sample of the session, and the meeting's words come later,
//  from the post-stop transcription pass.
//
//  An `actor` because it owns two per-channel buffers that both capture
//  callbacks append to at different cadences, and because a chunk must not be
//  detached while another callback is appending to it.
//
...
/// Derived per-chunk audio metrics that the speech gates evaluate. A value of
/// numbers — it never carries samples, which is what lets the gate-decision
/// record embed it wholesale and still be safe to log.
public struct AudioStats: Sendable {

    public let rms: Float
    public let peak: Float
    public let activeRatio: Float
    public let speechWindowRatio: Float
    public let strongWindowRatio: Float
    public let noiseFloorRMS: Float
    ...
    public init(
        ...
        peak: Float,
        activeRatio: Float,
        speechWindowRatio: Float,
        strongWindowRatio: Float,
        noiseFloorRMS: Float,
        ...
        self.speechWindowRatio = speechWindowRatio
        self.strongWindowRatio = strongWindowRatio
        self.noiseFloorRMS = noiseFloorRMS
        self.dynamicRangeDB = dynamicRangeDB
        self.crestFactor = crestFactor
    }

    /// Conservative evidence that the chunk contains actual speech rather
    /// than low-level residue or silence. The individual thresholds live on
    /// `GateTerm`, so the gate diagnostics report exactly the terms this
    /// decision runs on.
    public var hasClearSpeech: Bool {
        GateTerm.clearSpeech.allSatisfy { $0.passes(self) }
    }

    public var hasTranscribableSpeech: Bool {
        hasClearSpeech || GateTerm.loudFallback.allSatisfy { $0.passes(self) }
    }

    public var isMarginalSpeech: Bool {
    ...
    /// monitor's endpointing so gating and pause detection agree on what a
    /// window is.
    public static let probeSamples = Int(AudioConstants.sampleRate * 0.03)

    /// A window quieter than this (RMS) is treated as non-speech. Above the
    /// silence floor so room tone and breaths still register as a pause.
    public static let pauseRMS: Float = 0.01

    /// The gate-metric computation, exposed as a pure function so the tests
    /// can replay audio through the exact arithmetic the monitor uses.
    public static func compute(from samples: [Float]) -> AudioStats {
    ...
    public static func rms(_ samples: [Float], from start: Int, count: Int) -> Float {
        var sumSquares: Float = 0
...
public actor LiveInputMonitor {

    private static let log = Logger(subsystem: AppIdentity.logSubsystem, category: "LiveInputMonitor")
    ...
    /// gate-metric computation, so pauses and speech windows are measured
    /// identically.
    ...
    /// Receives one record per finalized-chunk gate decision.
    private let gateDiagnostics: any GateDiagnosticsSink

    /// The sink defaults to the permanent os.Logger diagnostic; tests and the
    /// input-health tracker inject their own to observe gate decisions
    /// in-process, and `FanOutGateDiagnosticsSink` feeds both at once.
    public init(gateDiagnostics: any GateDiagnosticsSink = OSLogGateDiagnosticsSink()) {
    ...
    public func ingest(_ frames: [Float], from channel: AudioChannel) {
    ...
    // MARK: - Capture-gap clock realignment
    ...
    public func noteCaptureGap(seconds: TimeInterval, on channel: AudioChannel) {
    ...
    /// Measures an already-detached chunk and records its gate decision, for
    /// both verdicts. This is the whole point of the monitor: nothing
    /// consumes the audio afterwards.
