...
//
//  The probe is here because it is pure and needs only this package. Its one
//  consumer, the offline echo-cancellation pre-pass, is NOT: that stage reads
//  Transcription's energy envelope as well as this package's AEC stage, and
//  the only package that sees both is Recording. It is disabled and
//  unfinished in any case — it eats double-talk, costing a fifth to a
//  quarter of the user's own words against a 0.90 preservation gate — so it
//  travels with whichever stack can hold it without a second copy of a
//  measured type. This probe's verdict is what that stage asks first.
//

import Foundation

public enum EchoBleedProbe {
    ...
    /// Envelope resolution. The 100 ms frames that suit comparing two
    /// channels' levels are far too coarse for a delay measured in tens of
    /// milliseconds, so this probe runs its own.
    public static let frameSeconds: TimeInterval = 0.005
    ...
    public static let windowSeconds: TimeInterval = 10
    ...
    public static let referenceFloor: Double = 0.01
    ...
    public static let bleedRatioCeiling: Double = 0.5
    ...
    public static let maxLagSeconds: TimeInterval = 0.4
    ...
    public static let coherenceFloor: Double = 0.35
    ...
    public static let agreementSeconds: TimeInterval = 0.025
    ...
    public static let agreementCount = 2
    ...
    public struct Verdict: Sendable, Equatable {
        ...
        public var bleedWindowStarts: [TimeInterval]
    }
    ...
    public static func run(mic: [Float], system: [Float]) -> Verdict? {
