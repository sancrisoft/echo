...
//  AudioTests
//
//  No silent dropout, and the earbuds combination surfaced rather than left
//  mute: the input-health classifier consumes the per-chunk gate-decision
//  record stream for both channels plus input-device signals, tracks
//  sustained-discard episodes, and drives the input-health notice — once per
//  episode, within the onset bound, never on silence, pauses or ambient
//  tone, clearing on recovery.
//
//  Table-driven over constructed record/event sequences — pure logic,
//  mirroring `EchoHandlingModeTests`. Time advances by the records' own
//  durations; no clocks. The stats profiles below are constructed values,
//  not audio: the classifier consumes derived metrics, and audio realism
//  lives in the fixture suites (the exact discrimination thresholds are
//  tunable against recorded fixtures once they are measured).
//
...
/// `InputHealthTracker`. Mutex-guarded like `CollectingGateSink`: deliveries
/// can arrive from the pipeline's executor in production, so the collector
/// honors the same thread-safety contract.
private final class CollectingHealthEffects: Sendable {

    private let storage = Mutex<[(generation: Int, effect: InputHealthClassifier.Effect)]>([])

    var deliver: @Sendable (Int, InputHealthClassifier.Effect) -> Void {
        { [self] generation, effect in
            storage.withLock { $0.append((generation, effect)) }
        }
    }
...
struct InputHealthClassifierTests {
    ...
    /// The silent-dropout signature: speech-like structure at an honest
    /// external-mic level — meter-visible energy, bursty windows, peaky
    /// crest — but too quiet for every gate level term, so the chunk is
    /// dropped on both channels. This is the profile that must accumulate.
    ...
    /// heuristic — the false-positive direction that is bound hardest:
    ...
    private static func transcribed(
        ...
        GateDecisionRecord(
            ...
            chunkDuration: duration,
            chunkStartOffset: offset,
            stats: transcribableSpeechStats
        )
    }
    ...
    // MARK: - False-positive bar: conservative in both directions
    ...
    /// nothing and emit zero effects. This is the silence/ambient fixture bar
    /// expressed at the classifier level.
    ...
    /// the same no-spurious-clear discipline the input-device lifecycle
    /// machine keeps.
    ...
    /// notice a genuinely gated-out mic owes the user, which must always
    /// fire within its onset bound.
    ...
    // MARK: - The Others channel: the earbuds case
    ...
    /// mid-recording (a profile switch after minutes of healthy Others
    /// audio): the same episode discipline applies wherever in the session it
    /// starts, so the Others channel is never silently mute.
    @Test func systemChannelSustainedDiscardRaisesTheUnsupportedCombinationNotice() {
        ...
        // Then every Others chunk starts gating out (the profile switch).
        var effects: [InputHealthClassifier.Effect] = []
        for _ in 0..<Int(InputHealthClassifier.onsetBound) {
            if let effect = classifier.handle(
        ...
        #expect(
            classifier.handle(.gateDecision(Self.transcribed(on: .system, at: offset)))
                == .clearHealthNotice(.system))
    }

    /// Channel independence: the mic and Others channels fail for different
    ...
    @Test func channelsAccumulateAndClearIndependently() {
        ...
        // Interleave: the mic discards activity while the Others channel
        // transcribes happily. Others recoveries must not end the mic episode.
        for _ in 0..<Int(InputHealthClassifier.onsetBound) {
            if let effect = classifier.handle(
                .gateDecision(Self.droppedActivity(on: .microphone, at: offset))
            ) {
        ...
        #expect(
            classifier.handle(.gateDecision(Self.transcribed(on: .microphone, at: offset)))
    ...
    /// mirrors the lifecycle machine treating a device change as a fresh
    /// capture attempt.) The device-lost degradation arrives through the
    /// same reset, so the mic-unavailable notice is never accompanied by a
    ...
    /// clears the Others channel's state — an earbuds-style system episode
    /// keeps its evidence across input flapping, so the Others dropout still
    /// surfaces while the user fiddles with input devices.
    ...
    // MARK: - Global structural sweep

    /// The classifier is observational — it raises and clears the
    /// input-health notice and never changes the audio path. Like the
    ...
    /// Recording hands the pipeline ONE sink; the fan-out keeps the permanent
    /// OSLog diagnostic and the input-health classifier both fed from the
    /// same gate-decision stream — every record to every sink, in order.
    ...
    @Test func trackerClassifiesOnlyWithinASession() {
        ...
        let tracker = InputHealthTracker(onEffect: effects.deliver)

        func thirtySixSecondsOfDiscard() {
            for i in 0..<3 {
                tracker.record(
                    Self.droppedActivity(
                        on: .microphone,
                        duration: 12,
                        at: TimeInterval(i * 12)
                    ))
            }
        }
    ...
    /// them (Recording drops stale deliveries by generation, like the
