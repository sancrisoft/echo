...
//  AudioTests
//
//  Fixture pairs through the real AEC engine, asserting post-cancellation
//  energy against the ingest path's speech gates. No model at all — this is
//  the signal-level layer, and the transcript-level layer that follows it
//  needs Transcription and therefore a package that can see both.
//
//  `.acceptance` because it replays real recordings, and each test gates on
//  its own scenario too: a checkout without `Fixtures/` skips with recording
...
struct AECSignalLevelTests {
    ...
    /// Convergence grace at the start of playback: the filter is allowed this
    /// long to adapt before anything is asserted about its residual.
    private static let convergenceGraceSeconds = 10.0
    private static let windowSamples = Int(AudioConstants.sampleRate)
    ...
    private func cancelled(_ scenario: String) throws -> (cleaned: [Float], raw: [Float]) {
        let pair = try AudioTestSupport.loadPair(scenario)
    ...
    // MARK: - Bleed-only (the speaker-bleed criterion, at signal level)
    ...
    // MARK: - Double-talk (the user's own speech survives cancellation)
    ...
    func doubleTalkPreservesUserSpeechEnergyDuringScriptedUtterances() throws {
        ...
        for span in Self.doubleTalkSpans {
            ...
            // Conservative preservation floor: the near-field user voice
            // dominates the raw span energy, so losing more than half of it
            // means cancellation ate user speech, not just bleed.
            #expect(
    ...
    // MARK: - Monologue (a silent far end causes no material attenuation)

    @Test(.enabled(if: Fixtures.available("monologue"), Comment(rawValue: Fixtures.instructions)))
    func monologueWithSilentFarEndIsNotMateriallyAttenuated() throws {
        ...
        while start + Self.windowSamples <= count {
            ...
            // A window with actual speech in the baseline.
            if rawRMS >= 0.01 {
                let cleanedRMS = SignalMetrics.rms(cleaned[start..<start + Self.windowSamples])
                if cleanedRMS < 0.7 * rawRMS {
                    violations.append(
                        "t=\(start / Int(AudioConstants.sampleRate))s raw=\(rawRMS) post-AEC=\(cleanedRMS)"
                    )
                }
            }
            start += Self.windowSamples
