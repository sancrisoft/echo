...
//  AudioTests
//
//  The declared sample rate, weighed against the wall clock. A Bluetooth
//  headset switching into call mode drops the output device to 24 kHz while
//  macOS keeps reporting 48 kHz, and a resampler built on that claim converts
//  the Others channel to half its real duration — the double-speed recording.
//
...
//  in CaptureGapTrackerTests: the assertions are about clock arithmetic, not
//  audio.
//
...
struct CaptureRateGuardTests {
    ...
    /// but it costs the Others channel 8% of every meeting and compresses its
