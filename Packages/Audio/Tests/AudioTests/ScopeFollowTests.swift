...
//  AudioTests
//
//  When a process-list change must touch the live scoped tap. The contract is
//  asymmetric on purpose — `nil` means "the tap already tells the truth, write
//  nothing", so unrelated process churn (and Core Audio's habit of
//  re-reporting the same list) never costs a property write; a non-nil set is
//  applied exactly once per real change, including the empty set when the app
//  fully exits, which is silence to record through, never a failure to handle.
//
...
struct ScopeFollowTests {
    ...
    @Test func helperAppearingGrowsTheSetInOneUpdate() {
        ...
        // the worst failure shape this feature has.
        let processes: [Entry] = [
    ...
    @Test func unrelatedProcessChurnNeverTouchesTheTap() {
        ...
        // constantly; none of it may reach the tap — scope logic runs on set
        // changes only.
        let processes: [Entry] = [
            Entry(object: 50, bundleID: "us.zoom.xos"),
    ...
    @Test func appFullyExitingUpdatesToTheEmptySetNotAFailure() {
        // "App died and I kept recording" is a legal state: the tap narrows
        // to nothing and records silence; a relaunch grows it back.
