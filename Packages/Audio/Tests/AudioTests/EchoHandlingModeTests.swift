...
//  AudioTests
//
//  The echo-handling mode machine's full table. Pure and exhaustive: the
//  machine is the only place that decides what happens to the mic signal on
//  a route or engine-health event, and the last test here is the executable
//  form of the rule that no such event may ever end a recording.
//
...
struct EchoHandlingModeTests {
    ...
    /// No event sequence may ever stop, fail, or lose a recording. The
    /// machine's `Effect` type models nothing but the degradation notice, so
    /// a stop outcome is unrepresentable; this sweep additionally verifies
