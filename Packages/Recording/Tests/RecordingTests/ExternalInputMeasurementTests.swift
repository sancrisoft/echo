...
//  RecordingTests
//
//  The external-input confirmation experiment, executable form. Replays the
...
//  from the same numbers the permanent gate diagnostics log carries.
//
//  Part B isolates the max-magnitude downmix: it replays the DJI native
//  multi-channel take downmixed BOTH ways — the old averaging and the new
//  max-magnitude — so the before/after of the downmix fix is measured on the
//  real device signal, which mic.wav (already max-magnitude) cannot show.
//
//  Diagnostic, not a pass/fail gate: it requires only that fixtures produced
//  records. The PoC wrote the full report to EchoTests/sp002-measure-report.txt
//  because Swift Testing swallowed `print` under xcodebuild; here the report is
//  an `Attachment`, like the transcription harnesses' — a test may not write
//  into the source tree (CLAUDE.md), and `swift test --attachments-path <dir>`
//  puts the file wherever the developer wants it.
//
//  `.acceptance` because it replays real recordings, and each part gates on its
//  own fixtures too: a checkout without `Fixtures/` skips with recording
...
struct ExternalInputMeasurementTests {
    ...
    @Test(
        .enabled(
            if: Fixtures.available("parity-baseline-builtin"),
    ...
    mutating func measureExternalInputGateVerdicts() async throws {
        ...
        emit("===== external-input gate-verdict measurement =====")
        ...
        for scenario in scenarios {
            ...
            let pair = try RecordingAcceptanceSupport.loadPair(scenario)
            await report(scenario: scenario, channel: .microphone, samples: pair.mic)
            await report(scenario: scenario, channel: .system, samples: pair.system)
        }

        emit("")
        emit("[B] downmix before/after on the DJI native multi-channel take")
        emit("    (averaging = the old downmix; maxmag = the shipping one)")
        for scenario in ["parity-dji-2cm", "parity-dji-20cm", "parity-dji-50cm", "external-ambient"] {
            guard RecordingAcceptanceSupport.micNativeAvailable(scenario) else {
        ...
        Attachment.record(lines.joined(separator: "\n"), named: "external-input-measurement.txt")

        // ---- Confirmed-finding regression guards ----
        // These pin what the measurement established, so the findings can't
        // silently rot: the DJI reaches parity only because of the
        ...
        let baseline = await transcribeSeconds(
            try RecordingAcceptanceSupport.loadPair("parity-baseline-builtin").mic,
            on: .microphone
        )
        ...
        for scenario in ["parity-dji-20cm", "parity-dji-50cm"] where Fixtures.available(scenario) {
            let dji = await transcribeSeconds(
                try RecordingAcceptanceSupport.loadPair(scenario).mic,
                on: .microphone
            )
        ...
        if RecordingAcceptanceSupport.micNativeAvailable("parity-dji-20cm") {
            let native = try RecordingAcceptanceSupport.loadNativeWAV(
                at: RecordingAcceptanceSupport.micNativeURL("parity-dji-20cm")
            )
            let averaged = await transcribeSeconds(
                Self.resampleTo16k(Self.averagingMono(native.channels), from: native.sampleRate),
                on: .microphone
            )
            let maxmag = await transcribeSeconds(
                Self.resampleTo16k(Self.maxMagMono(native.channels), from: native.sampleRate),
                on: .microphone
            )
        ...
        if Fixtures.available("external-ambient") {
            let ambient = await transcribeSeconds(
                try RecordingAcceptanceSupport.loadPair("external-ambient").mic,
                on: .microphone
            )
        ...
        if Fixtures.available("earbuds-in-out") {
            let teamCaptured = await transcribeSeconds(
                try RecordingAcceptanceSupport.loadPair("earbuds-in-out").system,
                on: .system
            )
    ...
    private mutating func report(scenario: String, channel: AudioChannel, samples: [Float]) async {
        let records = await Self.runGate(samples, on: channel)
        let takeSeconds = Double(samples.count) / AudioConstants.sampleRate
        guard !records.isEmpty else {
            emit(
                String(
                    format: "%@ %@ take=%.1fs NO RECORDS (all sub-chunk / never endpointed)",
    ...
    private mutating func compareNativeDownmix(scenario: String) async throws {
        let native = try RecordingAcceptanceSupport.loadNativeWAV(
            at: RecordingAcceptanceSupport.micNativeURL(scenario)
        )
    ...
    private static func resampleTo16k(_ mono: [Float], from rate: Double) -> [Float] {
        ...
        mono.withUnsafeBufferPointer { source in
            ...
            channelData[0].update(from: base, count: mono.count)
        }
