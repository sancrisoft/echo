...
//  RecordingTests
//
//  The auto-summary gate's pure eligibility rule: the newest-first scan runs
//  only while automatic summaries are on; an explicit user request front-runs
//  the scan and survives the toggle being off; the transcript-about-to-change
//  set defers both. Synthetic metas only.
//
//  Ported from the PoC unchanged in substance — the rule itself came across
//  as-is (ADR-006), so its table comes with it. `MeetingMeta` moved to
//  `Meetings` and gained defaulted fields, which the calls below simply do
//  not pass.
//
