...
//  `TranscriptUtterance.derive`/`isStandaloneBackchannel`. The backchannel
//  rows are the 2026-08-04 real meeting's mic channel. The summary
//  pipeline's rendered line format (the second consumer) is tested where
//  that pipeline lives.
//
...
struct TranscriptUtteranceTests {
    ...
    struct BackchannelClassifierTests {
        ...
        @Test(
            "real-meeting standalone rows classify as backchannel",
            arguments: [
        ...
        @Test(
            "a varied run of table words stays in the flow",
            arguments: [
        ...
        @Test(
            "real content never classifies as backchannel",
            arguments: [
    ...
    struct MergeTests {
        ...
        func sameSpeakerRunMerges() throws {
            ...
            #expect(utterance.text == "We reviewed the metrics and latency looks fine. Next step is the rollout.")
    ...
    // The PoC's "summary transcript rendering" sub-suite (SummarizationPipeline line format) moves with the
    // Summarization package; it is not part of EchoCore.
}
