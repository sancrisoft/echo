...
struct ErrorTraceLogTests {
    ...
    /// it. The log's directory does not exist up front — the log must create it.
    private func withTempLog<T>(_ body: (ErrorTraceLog, URL) async throws -> T) async throws -> T {
        let scratch = try TemporaryDirectory(prefix: "ErrorTraceLogTests")
        defer { scratch.remove() }
        let root = scratch.path("Logs")
    ...
    private func makeRecord(
        ...
        ErrorTraceRecord.make(
            ...
            category: "Tests",
            metadata: metadata,
            file: "EchoCoreTests/ErrorTraceLogTests.swift",
            line: 42,
            function: "test()",
            sessionID: UUID(),
            timestamp: timestamp,
