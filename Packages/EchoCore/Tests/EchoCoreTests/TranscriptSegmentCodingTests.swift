...
@Suite("TranscriptSegment coding (SP-007 S1, ADR-023)")
struct TranscriptSegmentCodingTests {
    ...
    func speakerEncodesAsPlainString() throws {
        let data = try Self.encoder.encode(try Self.makeSegment(channel: .microphone, speaker: .me))
    ...
    func stringFormRoundTrips() throws {
        let original = try Self.makeSegment(channel: .system, speaker: .teammates)
