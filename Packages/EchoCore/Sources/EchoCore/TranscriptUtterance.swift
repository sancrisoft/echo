...
//  EchoCore
//
//  Utterance merging and backchannel filtering as pure derivations over the
//  persisted segments. This is the one shared home: the transcript view derives
//  utterances at render time and the summary pipeline derives them when
//  assembling model input, so the two consumers can never disagree about what
//  was merged or filtered. Nothing here is ever persisted and nothing mutates
//  its input; the segment-level record stays the source of truth for dedup,
//  evidence citations and the accuracy harness.
//
...
public struct TranscriptUtterance: Identifiable, Hashable, Sendable {
    /// Stable identity: the first constituent segment's id. This is also the id
    /// the summary pipeline renders into transcript lines, so a cited utterance
    /// always resolves to a real persisted segment.
    public let id: UUID
    ...
    /// seam, so paragraphs never fuse across what chunking already treats as a
    /// break.
    public static let maxMergeGap: TimeInterval = 10
    ...
    public static let maxBackchannelVariety = 3

    /// The backchannel table, seeded from a real meeting's mic channel (es + en)
    /// plus close spelling variants. Entries are `normalizedWords` forms —
    /// lowercase, punctuation stripped, hyphens/commas split words ("Mm-hmm." →
    /// "mm hmm"). Tuning the filter is editing these rows.
    ...
    /// lowercase, split on anything non-alphanumeric (so punctuation, hyphens
    /// and apostrophes all become word breaks).
    ...
    static let longestBackchannelEntry =
        backchannelTokens
        .map { $0.split(separator: " ").count }.max() ?? 1

    /// True when the segment's normalized text consists ONLY of backchannel
    /// tokens drawn from a narrow vocabulary — a standalone acknowledgment that
    /// should not interrupt the other speaker's paragraph. The word sequence
    /// must partition into table entries ("ah ok" is one entry, "yeah okay" is
    /// two), so any non-token word keeps the segment in the flow however short
    /// it is.
    public static func isStandaloneBackchannel(_ text: String) -> Bool {
    ...
    /// The one shared derivation: persisted segments in, ordered utterances
    /// out. Pure — never mutates or persists anything; O(n) past the initial
    /// sort. The order of operations is fixed: sort by start, drop standalone
    /// backchannel FIRST, then merge same-speaker runs — so an acknowledgment
    /// never breaks the other speaker's run. A backchannel-only transcript
    /// derives to empty (the persisted record is untouched either way).
    public static func derive(from segments: [TranscriptSegment]) -> [TranscriptUtterance] {
        // Stable sort: tie equal starts by input position so the derivation is
        // deterministic regardless of stdlib sort guarantees.
