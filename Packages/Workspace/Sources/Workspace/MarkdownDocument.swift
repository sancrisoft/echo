...
//  Workspace
//
//  The pure parser behind the summary view. The on-device model streams a
//  Markdown document token by token and the UI re-parses the accumulated text
//  on every tick — so this parser's contract is leniency, not conformance:
//  `parse` accepts ANY string, including a prefix cut in the middle of a fence,
//  a table, or a `**bold` run, and always returns blocks a renderer can draw.
...
//  code, horizontal rules, and FLAT inline styles (bold/italic/code combine as
//  an OptionSet, they do not nest as a tree). Setext headings are out: a `---`
//  line is always a horizontal rule, so the parser never has to retroactively
//  reinterpret the previous line — which is what makes prefix-stable streaming
//  parses cheap. Anything unrecognized degrades to a paragraph rather than
//  being dropped.
//
...
public nonisolated enum MarkdownBlock: Hashable, Sendable {
...
public nonisolated struct MarkdownSpanStyle: OptionSet, Hashable, Sendable {
    public let rawValue: Int
    public init(rawValue: Int) { self.rawValue = rawValue }

    public static let bold = MarkdownSpanStyle(rawValue: 1 << 0)
    public static let italic = MarkdownSpanStyle(rawValue: 1 << 1)
    public static let code = MarkdownSpanStyle(rawValue: 1 << 2)
}

/// A run of text with one uniform style set.
public nonisolated struct MarkdownSpan: Hashable, Sendable {
    public var text: String
    public var style: MarkdownSpanStyle

    public init(text: String, style: MarkdownSpanStyle = []) {
        self.text = text
        self.style = style
    }
}
...
public nonisolated enum MarkdownCheckbox: Hashable, Sendable {
    case checked
    case unchecked
}

/// One list item, possibly carrying a nested child list.
public nonisolated struct MarkdownListItem: Hashable, Sendable {
    public var spans: [MarkdownSpan]
    public var checkbox: MarkdownCheckbox?
    public var children: [MarkdownListItem]
    public var childrenOrdered: Bool

    public init(
        spans: [MarkdownSpan], checkbox: MarkdownCheckbox? = nil, children: [MarkdownListItem] = [],
        childrenOrdered: Bool = false
    ) {
        self.spans = spans
...
/// A pipe table: one header row plus body rows, every row padded/truncated to
/// the header's width so the renderer can lay out a rigid grid.
public nonisolated struct MarkdownTable: Hashable, Sendable {
    public var header: [[MarkdownSpan]]
    public var rows: [[[MarkdownSpan]]]

    public init(header: [[MarkdownSpan]], rows: [[[MarkdownSpan]]]) {
        self.header = header
        self.rows = rows
    }
}
...
public nonisolated enum MarkdownDocument {
    ...
    public static func parse(_ text: String) -> [MarkdownBlock] {
        ...
        while i < lines.count {
            ...
            case .fence(let language):
                // Everything until the closing fence is verbatim — no trimming,
                // no Markdown. A missing close fence is the streaming case: the
                // rest of the input is code so far.
                i += 1
            ...
            case .listItem:
                ...
                // Anything else — blank line, heading, rule, plain prose at the
            ...
            case .pipeRow(let headerLine):
                ...
                // width so the renderer can lay out a rigid grid. A stream cut
                // mid-table simply yields the rows received so far.
    ...
    /// every cell is dashes with optional alignment colons, and at least one
    /// pipe is present (a bare "---" stays a horizontal rule).
    ...
    private static func buildItems(_ entries: [ListEntry], _ index: inout Int, level: Int) -> [MarkdownListItem] {
        ...
        while index < entries.count {
            ...
            if entry.level > level {
                ...
                if items.isEmpty {
                    // A deeper entry with no sibling yet to hang it on (the run
                    // began mid-indent): adopt the group at this level rather
    ...
    private enum Line {
        ...
        /// Contains "|": a table row — or plain prose, if no separator line
        /// follows.
        case pipeRow(String)
        /// Trimmed prose plus its indent, so lists can tell a nested
        /// continuation from prose that ends them.
        case text(indent: Int, content: String)
    }
    ...
    private static func classify(_ line: String) -> Line {
        ...
        // Horizontal rule: 3+ of one character among -, *, _ with nothing else
        // but spaces. Checked BEFORE the list marker so "- - -" is a rule, and
        // there are no setext headings to disambiguate against.
        ...
        // Indentation decides list nesting: every 2 spaces is one level, a tab
        // counts as 4 spaces (the model occasionally emits tabs).
        var indent = 0
        ...
        // prose. (A bare "-" is also prose: mid-stream it is just as likely the
        // first byte of a rule, so guessing "list" buys nothing.)
    ...
    /// Splits a task checkbox off a bullet's text: "[ ] ", "[x] ", "[X] " (or
    /// the bare box at end of line — the model may pause right there).
    ...
    /// the block. That last part is the streaming contract: while the model is
    /// mid-`**bold`, the text renders bold instead of flashing literal
    /// asterisks that vanish a tick later. Inside a code span every other
    /// delimiter is literal; only the closing backtick (or end of text) leaves
    /// it.
