//
//  MeetingListSelection.swift
//  Meetings
//
//  The meetings list's selection rules, kept out of any view so they can be
...
public enum MeetingListSelection {

    /// Where a keyboard command moves the selection.
    ...
    public static func destination(from selection: UUID?, move: Move, in ids: [UUID]) -> UUID? {
    ...
    /// a clean slate.
    public static func reconcile(_ selection: UUID?, with ids: [UUID]) -> UUID? {
    ...
    public static func selectionAfterRemoving(_ id: UUID, from ids: [UUID]) -> UUID? {
