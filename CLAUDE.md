...
| `Transcription` | `ParakeetModel` (identity, readiness, download), `TranscriptionPass` (the post-stop batch pass, segment shaping, `spanLevels`), `EnergyEnvelope`, `PassProgress`, `PassEvent`, `TranscriptionError`, `EchoDedupPolicy`; `TranscriptChunking` (`TranscriptChunk`/`ChunkAssembler`/`TranscriptChunker`/`ChunkingConfig`/`TokenEstimating`), which the design assigns to Summarization — see §3 | EchoCore, ModelDelivery, FluidAudio |
