...
| `CallDetection` | `CallAppCatalog` (the curated table, the matcher and the disabled-apps filter), `BrowserCatalog` (every installed browser, from LaunchServices), `MicCaptureClient`, `CallSessionMachine`/`CallDetectionTiming`/`IslandFace`; *pending*: the mic-activity monitor and the controller | EchoCore, Audio |
