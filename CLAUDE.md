...
| `Summarization` *(pending)* | `TranscriptChunking` (`TranscriptChunk`/`ChunkAssembler`/`TranscriptChunker`/`ChunkingConfig`/`TokenEstimating`); MLX engine, model lifecycle, pipeline, prompts | EchoCore, ModelDelivery |
