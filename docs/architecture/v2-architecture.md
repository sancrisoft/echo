...
- *Next layer (#85)*: `MicActivityMonitor` (the Core Audio shim) and `CallDetector` (`@Observable @MainActor`: `face`, `graceDeadline`, `appsInCall`, the taps, the three timers, and the requests it cannot serve itself).
